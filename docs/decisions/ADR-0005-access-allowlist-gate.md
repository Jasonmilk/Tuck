...
### D9｜语义混淆沿用 ADR-0004，本 ADR 只补**语料**热加载

ADR-0004 已决策 Cyber Camouflage（`mapping` category：实体 → 占位符）。**不另起炉灶**。
本 ADR 对它只做一件事：**检测规则（行业黑话语料）接入既有 `[hot_reload]`**，
使语料可在线更新而不重启。

**术语订正（2026-09-15）**：D9 初版写「映射表热加载」，不准确。必须区分两个东西：

| 对象 | 性质 | 可否热加载 |
|---|---|---|
| **`RuleSet`**（检测规则 / 行业黑话语料） | **配置数据** | ✅ 可热加载 |
| **`MappingTable`**（实体 → 占位符） | **运行时会话状态**，按 `X-Tuck-Session` 分会话、仅驻内存 | ❌ 不可 |

热替换 `MappingTable` 会毁掉会话内的占位符一致性 —— 已发往上游的脱敏文本将**无法还原**。
语料是配置数据，会话状态不是：只有前者该热加载。
...
| T9 | 语料热加载（D9）：复用 `[hot_reload]`，`RuleSet` 原子替换；新规则编译失败 ⇒ **保留旧规则 + 告警**（fail-closed 到旧值，不是空） | ⬜ |
